// @(#)root/tmva $Id: TMVAPhotons_region3.C,v 1.1 2008/05/28 09:18:46 delre Exp $
/**********************************************************************************
 * Project   : TMVA - a Root-integrated toolkit for multivariate data analysis    *
 * Package   : TMVA                                                               *
 * Root Macro: TMVAPhotons_region3                                                        *
 *                                                                                *
 ...
 *                                                                                *
 *    root -l TMVAPhotons_region3.C\(\"Fisher,Likelihood\"\)                              *
 *                                                                                *
...
void TMVAPhotons_region3( TString myMethodList = "" ) 
{
   ...
   //
   // mylinux~> root -l TMVAPhotons_region3.C\(\"myMethod1,myMethod2,myMethod3\"\)
   //
   // if you like to use a method via the plugin mechanism, we recommend using
   // 
   // mylinux~> root -l TMVAPhotons_region3.C\(\"P_myMethod\"\)
   ...
   std::cout << "Start Test TMVAPhotons_region3" << std::endl
             << "======================" << std::endl
             << std::endl;
   std::cout << "Testing all standard methods may take about 10 minutes of running..." << std::endl;

   // Create a new root output file.
   TString outfileName( "TMVAPhotons_region3.root" );
   ...
   TMVA::Factory *factory = new TMVA::Factory( "TMVAPhotons_region3", outputFile, Form("!V:%sColor", gROOT->IsBatch()?"!":"") );
   ...
   TString fname_signal = "/u1/delre/mixsignal.root";
   TString fname_background = "/u1/delre/mixbkg.root";
   // first we try to find tmva_example.root in the local directory
   std::cout << "--- TMVAPhotons_region3  : accessing " << fname_ signal<< "   " << fname_background <<std::endl;
   input_signal = TFile::Open( fname_signal );
   input_background = TFile::Open( fname_background );

   if (!input_signal) {
     std::cout << "ERROR: could not open " << fname_signal << " data file" << std::endl;
     exit(1);
   }
   if (!input_background) {
     std::cout << "ERROR: could not open " << fname_background << " data file" << std::endl;
     exit(1);
   }
   
   TTree *signal     = (TTree*)input_signal->Get("TreeS");
   TTree *background = (TTree*)input_background->Get("TreeB");
   ...
   factory->AddVariable("ntrkiso", 'I');
   factory->AddVariable("ptiso", 'F');
//    factory->AddVariable("ptisoatecal", 'F');
   factory->AddVariable("hcalovecal", 'F');
   factory->AddVariable("sMajMaj", 'F');
   factory->AddVariable("sMinMin", 'F');
...
//    TCut mycutb = ""; // for example: TCut mycutb = "abs(var1)<0.5";
   TCut mycuts = "pt<200&&pt>100"; // for example: TCut mycuts = "abs(var1)<0.5 && abs(var2-0.5)<1";
   TCut mycutb = "pt<200&&pt>100"; // for example: TCut mycutb = "abs(var1)<0.5";
...
//    factory->PrepareTrainingAndTestTree( mycuts, mycutb, "NSigTrain=3000:NBkgTrain=3000:SplitMode=Random:NormMode=NumEvents:!V" );  
   ...
   // tell the factory to use all remaining events in the trees after training for testing:
   factory->PrepareTrainingAndTestTree( mycuts, mycutb, "NSigTrain=20000:NBkgTrain=20000:SplitMode=Random:NormMode=NumEvents:!V" );  
   ...
   std::cout << "==> TMVAPhotons_region3 is done!" << std::endl;      
