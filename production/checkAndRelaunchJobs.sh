...
#	     crab -forceResubmit ${jobsCrashed} -c $crabdir > ${task}_resubmit_crashed.log&
	      ./submitTask.py -l  ${jobsCrashed} -t $crabdir -r -s 1 > ${task}_resubmit_crashed.log&
	fi
...
#	    crab -forceResubmit ${jobsAborted}  -c $crabdir > ${task}_resubmit_aborted.log&
	    ./submitTask.py -l  ${jobsAborted} -t $crabdir -r -s 1 > ${task}_resubmit_aborted.log&
	fi

	jobsCancelled=`cat ${task}_status.log | grep -i -A 2 "Cancelled" |  grep -A2 ">>>>>>>>>" | grep "List of jobs Cancelled:" | sed -e "s%.*List of jobs Cancelled: %%g" | awk '{printf "%s,",$1}' | sed -e 's%,$%%g' | awk -f ${STARTDIR}/expandJobsList.awk | sed -e 's%,$%%g'` 
	if [ "${jobsCancelled}AAA" != "AAA" ]; then
	    echo ">>>> Relaunching cancelled jobs for task $task: jobs ${jobsCancelled}"
#	    crab -forceResubmit ${jobsCancelled}  -c $crabdir > ${task}_resubmit_cancelled.log&
	    ./submitTask.py -l  ${jobsCancelled} -t $crabdir -r -s 1  > ${task}_resubmit_cancelled.log&
	fi
	...
	echo ">>>> Getting list of produced files for task $task in ${CASTORDIR}/${outDir}"
	xrd cmseos.fnal.gov dirlist ${CASTORDIR}/${outDir} > /tmp/${outDir}.list 
...
#	      FILEOK=`/afs/cern.ch/project/eos/installation/0.1.0-22d/bin/eos.select ls ${CASTORDIR}/${outdir} | grep  output_${job} | awk '{print $9}'`
	      FILEOK=`cat /tmp/${outDir}.list | grep  output_${job} | awk '{print $9}'`
