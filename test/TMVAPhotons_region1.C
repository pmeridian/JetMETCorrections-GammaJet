// @(#)root/tmva $Id: TMVAPhotons_region1.C,v 1.1 2008/05/28 09:18:46 delre Exp $
/**********************************************************************************
 * Project   : TMVA - a Root-integrated toolkit for multivariate data analysis    *
 * Package   : TMVA                                                               *
 * Root Macro: TMVAPhotons_region1                                                        *
 *                                                                                *
 ...
 *                                                                                *
 *    root -l TMVAPhotons_region1.C\(\"Fisher,Likelihood\"\)                              *
 *                                                                                *
...
void TMVAPhotons_region1( TString myMethodList = "" ) 
{
   ...
   //
   // mylinux~> root -l TMVAPhotons_region1.C\(\"myMethod1,myMethod2,myMethod3\"\)
   //
   // if you like to use a method via the plugin mechanism, we recommend using
   // 
   // mylinux~> root -l TMVAPhotons_region1.C\(\"P_myMethod\"\)
   ...
   std::cout << "Start Test TMVAPhotons_region1" << std::endl
             << "======================" << std::endl
             << std::endl;
   std::cout << "Testing all standard methods may take about 10 minutes of running..." << std::endl;

   // Create a new root output file.
   TString outfileName( "TMVAPhotons_region1.root" );
   ...
   TMVA::Factory *factory = new TMVA::Factory( "TMVAPhotons_region1", outputFile, Form("!V:%sColor", gROOT->IsBatch()?"!":"") );
   ...
   std::cout << "--- TMVAPhotons_region1  : accessing " << fname_ signal<< "   " << fname_background <<std::endl;
...
//    TCut mycutb = ""; // for example: TCut mycutb = "abs(var1)<0.5";
   TCut mycuts = "pt<60&&pt>30"; // for example: TCut mycuts = "abs(var1)<0.5 && abs(var2-0.5)<1";
   TCut mycutb = "pt<60&&pt>30"; // for example: TCut mycutb = "abs(var1)<0.5";
...
//    factory->PrepareTrainingAndTestTree( mycuts, mycutb, "NSigTrain=3000:NBkgTrain=3000:SplitMode=Random:NormMode=NumEvents:!V" );  
   ...
   std::cout << "==> TMVAPhotons_region1 is done!" << std::endl;      
